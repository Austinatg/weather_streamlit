...
# Predict Button
    if st.button("🔮 Predict Weather"):
        try:
            ...
            # Convert Precip Type back to categorical values
            # if "preciptype" in feature_names:
            #     precip_predictions = ["None" if abs(val)<abs(val-1) else "Rain" for val in prediction[:, 0]]
            # else:
            #     precip_predictions = ["Rain" if abs(val)<abs(val-1) else "Snow" for val in prediction[:, 0]]
